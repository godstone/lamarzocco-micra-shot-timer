...
- **main.cpp** — everything runs in one `loop()` on core 1: mode state machine
  ...
  idle pages (status / stats / backflush), 6-page dev/settings mode (info; demo+bootlog;
  rotate+setup-portal+reset; theme; dark-light; setup-QR), demo gallery, boot-log console.
...
- Button geometry (config.h): `BTN_A_Y`/`BTN_B_Y` = confirm modals + backflush page;
  `DEV_BTN_A/B/C_Y` = stacked buttons on both dev pages; `WIFI_CANCEL_Y/H` = setup-screen
  CANCEL. Touch hit-boxes get a +12 px margin.
...
- Secrets flow: `secrets.h` is OPTIONAL — it only seeds NVS on first boot. Runtime settings live
  in NVS: WiFi list ("wifis"), LM account ("lmacct": user/pass/serial/tz), installation key
  ("lmstats"). Devices provision phone-only via the captive portal (WiFiManager params); the
  serial is auto-discovered from `GET /things` when left empty. Developer seed path:
  `tools/preflight/.env` → `tools/gen_secrets.py` → `firmware/include/secrets.h` (git-ignored).
  Never commit credentials, keys, or the generated `logo_lm.h`/`machine_off.h`
  (trademarked-artwork derived).
